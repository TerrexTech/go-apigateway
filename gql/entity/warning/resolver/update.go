...
var Update = func(params graphql.ResolveParams) (interface{}, error) {
	...
	if err != nil {
		err = errors.Wrap(err, "WarningUpdateResolver: Error marshalling into JSON")
		return nil, err
	}
	...
	cid, err := uuuid.NewV4()
	if err != nil {
		err = errors.Wrap(err, "WarningUpdateResolver: Error generating UUID for cid")
		return nil, err
	}
	eventID, err := uuuid.NewV4()
	if err != nil {
		err = errors.Wrap(err, "WarningUpdateResolver: Error generating UUID for UpdateWarning-Event")
		return nil, err
	}
	...
	if err != nil {
		err = errors.Wrap(err, "WarningUpdateResolver: Error creating ConsumerIO for WarningUpdateResolver")
		return nil, err
	}
	...
	case kr := <-krChan:
		UpdateWarnResp := handleUpdateWarnResponse(kr)
		if UpdateWarnResp != nil {
			if UpdateWarnResp.Err == nil {
				return UpdateWarnResp.Result, nil
			}
			ae := UpdateWarnResp.Err
			err = errors.Wrap(ae.Err, "WarningUpdateResolver: UpdateWarning Error")
			log.Println(err)
			outErr := fmt.Errorf("%d: UpdateWarning Error", ae.Code)
			return nil, outErr
		}
	}
...
func handleUpdateWarnResponse(kr esmodel.KafkaResponse) *response.ResolverResponse {
	if kr.Error != "" {
		err := errors.Wrap(errors.New(kr.Error), "UpdateWarningResponseHandler: Error in KafkaResponse")
