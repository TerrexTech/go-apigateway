...
var DeleteItem = func(params graphql.ResolveParams) (interface{}, error) {
	consTopic := os.Getenv("KC_TOPIC_SHIPMENT_CMD")
	...
	case doc := <-docChan:
		registerResp := handleDeleteItemResponse(doc)
		return registerResp, nil
	}
}

func handleDeleteItemResponse(doc esmodel.Document) map[string]interface{} {
	if doc.Error != "" {
	...
	err := json.Unmarshal(doc.Data, &item)
	if err != nil {
		err = errors.Wrap(
			err,
