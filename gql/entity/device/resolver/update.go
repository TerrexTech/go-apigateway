...
var Update = func(params graphql.ResolveParams) (interface{}, error) {
	...
	if err != nil {
		err = errors.Wrap(err, "DeviceUpdateResolver: Error marshalling into JSON")
		return nil, err
	}
	...
	cid, err := uuuid.NewV4()
	if err != nil {
		err = errors.Wrap(err, "DeviceUpdateResolver: Error generating UUID for cid")
		return nil, err
	}
	eventID, err := uuuid.NewV4()
	if err != nil {
		err = errors.Wrap(err, "DeviceUpdateResolver: Error generating UUID for UpdateDevice-Event")
		return nil, err
	}
	...
	if err != nil {
		err = errors.Wrap(err, "DeviceUpdateResolver: Error creating ConsumerIO for DeviceUpdateResolver")
		return nil, err
	}
	...
	case kr := <-krChan:
		UpdateDeviceResp := handleUpdateDeviceResponse(kr)
		if UpdateDeviceResp != nil {
			if UpdateDeviceResp.Err == nil {
				return UpdateDeviceResp.Result, nil
			}
			ae := UpdateDeviceResp.Err
			err = errors.Wrap(ae.Err, "DeviceUpdateResolver: UpdateDevice Error")
			log.Println(err)
			outErr := fmt.Errorf("%d: UpdateDevice Error", ae.Code)
			return nil, outErr
		}
	}
...
func handleUpdateDeviceResponse(kr esmodel.KafkaResponse) *response.ResolverResponse {
	if kr.Error != "" {
		err := errors.Wrap(errors.New(kr.Error), "UpdateDeviceResponseHandler: Error in KafkaResponse")
