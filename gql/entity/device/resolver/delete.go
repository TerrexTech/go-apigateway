...
var Delete = func(params graphql.ResolveParams) (interface{}, error) {
	...
	if err != nil {
		err = errors.Wrap(err, "DeviceDeleteResolver: Error marshalling into JSON")
		return nil, err
	}
	...
	cid, err := uuuid.NewV4()
	if err != nil {
		err = errors.Wrap(err, "DeviceDeleteResolver: Error generating UUID for cid")
		return nil, err
	}
	eventID, err := uuuid.NewV4()
	if err != nil {
		err = errors.Wrap(err, "DeviceDeleteResolver: Error generating UUID for DeleteDevice-Event")
		return nil, err
	}
	...
	if err != nil {
		err = errors.Wrap(err, "DeviceDeleteResolver: Error creating ConsumerIO for DeviceDeleteResolver")
		return nil, err
	}
	...
	case kr := <-krChan:
		...
		if DeleteDonateResp != nil {
			...
			err = errors.Wrap(ae.Err, "DeviceDeleteResolver: DeleteDevice Error")
			log.Println(err)
			outErr := fmt.Errorf("%d: DeleteDevice Error", ae.Code)
			return nil, outErr
		}
	}
...
func handleDeleteDonateResponse(kr esmodel.KafkaResponse) *response.ResolverResponse {
	if kr.Error != "" {
		err := errors.Wrap(errors.New(kr.Error), "DeleteDeviceResponseHandler: Error in KafkaResponse")
