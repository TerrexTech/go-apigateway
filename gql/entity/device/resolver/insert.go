...
var Insert = func(params graphql.ResolveParams) (interface{}, error) {
	...
	deviceJSON, err := json.Marshal(params.Args)
	if err != nil {
		err = errors.Wrap(err, "DeviceInsertResponseHandler: Error marshalling credentials into JSON")
		return nil, err
	}
	...
	cid, err := uuuid.NewV4()
	if err != nil {
		err = errors.Wrap(err, "DeviceInsertResponseHandler: Error generating UUID for cid")
		return nil, err
	}
	eventID, err := uuuid.NewV4()
	if err != nil {
		err = errors.Wrap(err, "DeviceInsertResponseHandler: Error generating UUID for InsertInventory-Event")
		return nil, err
	}
	...
	if err != nil {
		err = errors.Wrap(err, "DeviceInsertResponseHandler: Error creating ConsumerIO")
		return nil, err
	}
	...
	case kr := <-krChan:
		...
		if insertDonateResp != nil {
			...
			err = errors.Wrap(insertDonateErr.Err, "DeviceInsertResponseHandler: InsertDevice Error")
			log.Println(err)
			outErr := fmt.Errorf("%d: InsertDevice Error", insertDonateErr.Code)
			return nil, outErr
		}
	}
...
func handleInsertDonateResponse(kr esmodel.KafkaResponse) *response.ResolverResponse {
	if kr.Error != "" {
		err := errors.Wrap(
			errors.New(kr.Error),
			"DeviceInsertResponseHandler: Error in KafkaResponse",
		)
		log.Println(err)
	...
	if err != nil {
		err = errors.Wrap(
			err,
			"DeviceInsertResponseHandler: "+
				"Error while Unmarshalling KafkaResponse into device",
		)
		log.Println(err)
