...
var genericQuery = func(serviceAction string, params graphql.ResolveParams) (interface{}, error) {
	consTopic := os.Getenv("KAFKA_CONSUMER_TOPIC_DEVICE")
	...
	if err != nil {
		err = errors.Wrap(err, "DeviceQueryResolver: Error marshalling params into JSON")
		return nil, err
	}
	...
	cid, err := uuuid.NewV4()
	if err != nil {
		err = errors.Wrap(err, "DeviceQueryResolver: Error generating UUID for cid")
		return nil, err
	}
	uuid, err := uuuid.NewV4()
	if err != nil {
		err = errors.Wrap(err, "DeviceQueryResolver: Error generating UUID for Query-Event")
		return nil, err
	}
	...
	if err != nil {
		err = errors.Wrap(err, "DeviceQueryResolver: Error creating ConsumerIO")
		return nil, err
	}
	// Timeout Context
...
authResponseLoop:
	...
	for {
		...
		case msg := <-cio:
			authRes := handleDeviceQueryResponse(msg)
			if authRes != nil {
				if authRes.Err == nil {
...
func handleDeviceQueryResponse(kr esmodel.KafkaResponse) *response.ResolverResponse {
	if kr.Error != "" {
		err := errors.New(kr.Error)
		err = errors.Wrap(err, "DeviceQueryResponseHandler: Error in KafkaResponse")
	...
	if err != nil {
		err = errors.Wrap(
			err,
			"DeviceQueryResponseHandler: Error while Unmarshalling device into KafkaResponse",
		)
		log.Println(err)
	...
	for i, r := range result {
		...
		if !assertOK {
			...
			err = errors.Wrapf(
				err,
				"DeviceQueryResponseHandler: Error asserting item at index: %d", i,
			)
			log.Println(err)
			krerr := gwerrors.NewKRError(err, gwerrors.InternalError, err.Error())
			return &response.ResolverResponse{
